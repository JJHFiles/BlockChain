...
public class NodeListener extends Thread {
    ...
    // Blockchains almacenadas hasta su serialización
    ...
    private final String BLOCKCHAIN_FILE = "src\\ValidatorServerSSL1\\persistentBlockchain.dat";
    ...
    public void registryNewBlockchain(TempBlock tb) {
        ...
        try {
            ...
            // Primero se comprueba si existe un fichero serializado de blockchains, si no es asi se crea uno.
            ...
            for (int x = 0; x < blockchain.size(); x++) {
                ...
                if ((blockchain.get(x).getBlocks().size() < tempBChain.get(x).getBlocks().size())) {
                    ...
                    for (int y = blockchain.get(x).getBlocks().size(); y < tempBChain.get(x).getBlocks().size(); y++) {
                        ...
                        // Datos del bloque nuevo que se copiara en la blockchain local
